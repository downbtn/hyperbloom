 ...
 * @date 2025
 *
 * This source file is part of an example system for MITRE's 2025 Embedded
 * System CTF (eCTF). This code is being provided only for educational purposes
 * for the 2025 MITRE eCTF competition, and may not meet MITRE standards for
 ...
 *  @param should_Ack True if the decoder should expect an ACK. This should be
 * false for debug and ACK messages.
 *
 *  @return 0 on success. A negative value on error.
 */
int write_bytes(const void *buf, uint16_t len, bool should_ack)
{
    for (int i = 0; i < len; i++) {
        if (i % 256 == 0 &&
            i != 0) { // Expect an ACK after sending every 256 bytes
...
/** @brief Write len bytes to UART in hex. 2 bytes will be printed for every
 * byte.
 *
...
int write_hex(msg_type_t type, const void *buf, size_t len)
{
    ...
    for (i = 0; i < len; i++) {
        if (i % (256 / 2) == 0 && i != 0) {
            if (type != DEBUG_MSG && read_ack() < 0) {
                // If the block was not ack'd, don't send the rest of the
                // message
                return -1;
            }
        }
...
int write_packet(msg_type_t type, const void *buf, uint16_t len)
{
    ...
    if (len > 0) {
        ...
        // If we still need to ACK the last block (write_bytes does not handle
        // the final ACK)
        if (type != DEBUG_MSG && read_ack() < 0) {
            return -1;
        }
 ...
 *
 *  @param cmd A pointer to the resulting opcode of the packet. Must not be
 * null.
    ...
    if (header.cmd != ACK_MSG) {
        ...
        if (header.len && buf != NULL) {
            ...
            if (header.len < buf_size) {
                read_length = header.len;
            } else {
                read_length = buf_size;
            }

            if (read_bytes(buf, read_length) < 0) {
                return -1;
            }
        }
        if (header.len) {
            if (write_ack() <
                0) { // ACK the final block (not handled by read_bytes)
                return -1;
            }
        }
    }
