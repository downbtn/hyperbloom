 ...
 * @date    2025
 *
 * @copyright Copyright (c) 2025 The MITRE Corporation, with changes (c) 2025
 * Daniel Ha
 */
...
_Static_assert(sizeof(decrypted_subscription_t) == 56,
               "sizeof(decrypted_subscription_t)");
...
_Static_assert(sizeof(subscription_update_packet_t) == 104,
...
typedef struct {
    uint32_t
        first_boot; // if set to FLASH_FIRST_BOOT, device has booted before.
...
int update_subscription(pkt_len_t pkt_len, subscription_update_packet_t *update)
{
    ...
    const uint32_t size_to_mac =
        sizeof(update->iv) + sizeof(update->encrypted_sub);
    _Static_assert(size_to_mac == 72);
    compute_hmac((const uint8_t *)update,
                 sizeof(update->iv) + sizeof(update->encrypted_sub),
                 SUBSCRIPTION_KEY, hmac);
    ...
    decrypt_sym(update->encrypted_sub, sizeof(decrypted_data), update->iv,
                SUBSCRIPTION_KEY, decrypted_data);

    decrypted_subscription_t *decrypted_sub =
        (decrypted_subscription_t *)decrypted_data;
    ...
    for (i = 0; i < MAX_CHANNEL_COUNT; i++) {
        if (decoder_status.subscribed_channels[i].id ==
                decrypted_sub->channel ||
            ...
            decoder_status.subscribed_channels[i].end_timestamp =
                decrypted_sub->end;
            memcpy(&decoder_status.subscribed_channels[i].key,
                   decrypted_sub->key, sizeof(decrypted_sub->key));
            break;
        }
    }
...
int decode(pkt_len_t pkt_len, frame_packet_t *new_frame)
{
    ...
    const uint32_t size_to_mac = sizeof(new_frame->iv) +
                                 sizeof(new_frame->channel) +
                                 sizeof(new_frame->encrypted_frame);
    ...
    decrypt_sym((const uint8_t *)new_frame->encrypted_frame,
                sizeof(new_frame->encrypted_frame),
                (const uint8_t *)new_frame->iv, key, (uint8_t *)&dec);
...
void init()
{
    ...
    flash_simple_read(FLASH_STATUS_ADDR, &decoder_status,
                      sizeof(flash_entry_t));
    if (decoder_status.first_boot != FLASH_FIRST_BOOT) {
        /* If this is the first boot of this decoder, mark all channels as
         * unsubscribed. This data will be persistent across reboots of the
         * decoder. Whenever the decoder processes a subscription update, this
...
int main(void)
{
    ...
    while (1) {
        ...
        switch (cmd) {
            ...
            case SUBSCRIBE_MSG:
                STATUS_LED_YELLOW();
                update_subscription(pkt_len,
                                    (subscription_update_packet_t *)uart_buf);
                break;
